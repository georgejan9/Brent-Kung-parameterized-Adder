...
This repository contains a complete implementation of a **Brent-Kung Parallel Prefix Adder** using **Verilog HDL**. The Brent-Kung adder is a high-speed adder architecture that reduces carry propagation delay using a tree-based prefix structure, making it suitable for fast digital arithmetic circuits.  

The project includes all required modules such as **Generate/Propagate Logic**, **Carry Determination**, and **Sum Logic**, along with a full **testbench** for simulation and verification.

---

## 📌 Features

- Full Verilog implementation of the **Brent-Kung Adder**  
- Implements **Generate (G)** and **Propagate (P)** logic  
- Parallel prefix tree carry computation for high speed  
- Modular and well-organized design  
...
2. Use the Brent-Kung prefix tree to compute carry signals.  
3. Compute sum bits using:  
...
The testbench `Brent_Kung_tb.v` applies multiple random test vectors for inputs **A**, **B**, and **Cin**, then compares the output with the expected value computed using behavioral addition.  

The simulation outputs:  
- Input values: `A`, `B`, `Cin`  
- Actual output from the Brent-Kung Adder  
- Correct output  
- Pass/Fail result  
